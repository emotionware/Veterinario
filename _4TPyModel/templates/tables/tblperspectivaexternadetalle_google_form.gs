...
var db = 'veterinario';
function formbuild() { 
var form = FormApp.create("Perspectiva Detalle"); 
var conexion=conectar_mysql(); //realizo conexion con mysql



var guardar = SpreadsheetApp.create('Perspectiva Detalle');
...
item.setTitle("Nombre Reporte");


var form = FormApp.openById(form.getId());
var item = form.addTextItem(); //
item.setTitle("Url Reporte Pdf");
...
var sentencia="insert into tblperspectivaexternadetalle(NOMBRE_REPORTE,URL_REPORTE_PDF,OBSERVACIONES) values ('"  + itemResponses[0].getResponse() + "','"  + itemResponses[1].getResponse() + "','"  + itemResponses[2].getResponse() + "')";
